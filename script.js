document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        // 聊天标题点击进入资料卡 (已移除，改为点击头像)
        // const chatTitle = document.getElementById('chat-title');
        // if (chatTitle) chatTitle.addEventListener('click', openAiProfile);
...
4. 直接返回内容文本，不要包含任何解释、引号或前缀后缀。`;
    ...
    window.openAiProfile = function() {
        ...
        document.getElementById('ai-profile-name').textContent = contact.remark || contact.name;
        document.getElementById('ai-profile-id').textContent = `ID: ${contact.id}`; // 模拟微信号
    ...
    function openAiMoments() {
        // 暂时只提示，或者跳转到朋友圈Tab并筛选（如果支持筛选）
        // 这里简单实现：跳转到朋友圈Tab，但目前朋友圈Tab是显示所有的
        // 可以在未来实现只看某人的朋友圈
        
        // 简单提示
        // alert('朋友圈功能正在完善中，将跳转到全部动态');
