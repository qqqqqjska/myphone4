document.addEventListener('DOMContentLoaded', () => {
    ...
    const closeWalletRechargeBtn = document.getElementById('close-wallet-recharge');
    ...
    function setupEventListeners() {
        ...
        if (chatMoreBtn && chatMorePanel) {
            ...
            document.addEventListener('click', (e) => {
                if (!chatMorePanel.classList.contains('hidden') && 
        ...
        if (addMomentBtn) {
            const handleAddMoment = (e) => {
                ...
                if (e.type === 'touchstart') {
                    e.preventDefault(); // 防止触发后续的 click 和页面滚动
                }
                // 统一打开默认发布界面（图文）
                openPostMoment(false);
            };

            // 同时绑定 click 和 touchstart 以兼容移动端和桌面端
            addMomentBtn.addEventListener('click', handleAddMoment);
            addMomentBtn.addEventListener('touchstart', handleAddMoment, { passive: false });
        }

        // 动态页返回按钮
        const momentsBackBtn = document.getElementById('moments-back-btn');
        if (momentsBackBtn) {
            const handleMomentsBack = (e) => {
                if (e.type === 'touchstart') {
                    e.preventDefault();
                }
                // 切换回联系人列表 tab
            ...
            momentsBackBtn.addEventListener('click', handleMomentsBack);
            momentsBackBtn.addEventListener('touchstart', handleMomentsBack, { passive: false });
        }
...
请生成 1 到 3 条回复，每条回复之间用 '|||' 分隔。不要包含其他解释性文字。`;
    ...
    function renderWallet() {
        ...
        state.wallet.transactions.forEach(t => {
            ...
            item.innerHTML = `
                <div class="trans-icon" style="background-color: ${isIncome ? '#07C160' : '#FF9500'}">
                ...
                <div class="trans-amount ${amountClass}">${amountPrefix}${parseFloat(t.amount).toFixed(2)}</div>
            `;
    ...
    function renderRechargeOptions() {
        const grid = document.getElementById('recharge-options');
        grid.innerHTML = '';
        ...
        options.forEach(amount => {
            ...
            btn.className = 'recharge-option';
            btn.textContent = `¥${amount}`;
            btn.onclick = () => {
                document.querySelectorAll('.recharge-option').forEach(el => el.classList.remove('selected'));
                btn.classList.add('selected');
                document.getElementById('custom-recharge-amount').value = ''; // 清空自定义金额
            };
    ...
    function handleRecharge() {
        ...
        const customAmount = document.getElementById('custom-recharge-amount').value;
        
        if (selectedOption) {
            amount = parseFloat(selectedOption.textContent.replace('¥', ''));
