document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventListeners() {
        ...
        if (chatMoreBtn && chatMorePanel) {
            ...
            chatMorePanel.querySelectorAll('.more-item').forEach(item => {
                item.addEventListener('click', (e) => {
                    // 如果是照片、拍摄、转账或记忆按钮，不执行通用逻辑（由单独的监听器处理）
                    if (item.id === 'chat-more-photo-btn' || item.id === 'chat-more-camera-btn' || item.id === 'chat-more-transfer-btn' || item.id === 'chat-more-memory-btn') return;
        ...
        if (stickerBtn && stickerPanel) {
            stickerBtn.addEventListener('click', (e) => {
                ...
                if (!stickerPanel.classList.contains('hidden')) {
                    // 重置管理模式
                    state.isStickerManageMode = false;
        ...
        // 表情包导入相关
        const importStickerModal = document.getElementById('import-sticker-modal');
        const closeImportStickerBtn = document.getElementById('close-import-sticker');
        const saveStickerImportBtn = document.getElementById('save-sticker-import-btn');
        const addStickerCategoryBtn = document.getElementById('add-sticker-category-btn');

        if (addStickerCategoryBtn) {
            addStickerCategoryBtn.addEventListener('click', () => {
                document.getElementById('sticker-category-name').value = '';
                document.getElementById('sticker-import-text').value = '';
                importStickerModal.classList.remove('hidden');
            });
        }

        if (closeImportStickerBtn) {
            closeImportStickerBtn.addEventListener('click', () => importStickerModal.classList.add('hidden'));
        }

        if (saveStickerImportBtn) {
            saveStickerImportBtn.addEventListener('click', handleSaveStickerImport);
        }
        ...
        const stickerSelectAllBtn = document.getElementById('sticker-select-all-btn');
        const stickerDeleteBtn = document.getElementById('sticker-delete-btn');
        const stickerFinishBtn = document.getElementById('sticker-finish-btn');
        ...
        if (stickerManageBtn) {
            stickerManageBtn.addEventListener('click', (e) => {
                e.stopPropagation();
                e.preventDefault();
                state.isStickerManageMode = true;
                state.selectedStickers.clear();
                updateStickerToolbar();
                renderStickerPanel();
            });
        ...
        if (stickerFinishBtn) {
            stickerFinishBtn.addEventListener('click', () => {
                state.isStickerManageMode = false;
                state.selectedStickers.clear();
                updateStickerToolbar();
                renderStickerPanel();
        ...
        if (stickerSelectAllBtn) {
            stickerSelectAllBtn.addEventListener('click', handleStickerSelectAll);
        }
...
示例: {"nickname": "小王", "wxid": "wang_123", "signature": "今天天气不错"}`;
    ...
    function renderChatHistory(contactId) {
        ...
        messages.forEach(msg => {
            appendMessageToUI(msg.content, msg.role === 'user', msg.type || 'text', msg.description);
        });
    ...
    function sendMessage(text, isUser, type = 'text', description = null) {
        ...
        appendMessageToUI(text, isUser, type, description);
        scrollToBottom();
    ...
    function appendMessageToUI(text, isUser, type = 'text', description = null) {
...
- 如果你想更改自己的资料（网名、微信号、个性签名），请在回复最后另起一行输出：
  ...
  (可以同时使用多个更改指令)
${contact.showThought ? '- 请在回复的最后（所有指令之后），另起一行输出你的内心独白（心声），格式为：THOUGHT: 内容' : ''}
...
${contact.showThought ? '7. 必须输出 THOUGHT: 内容，描述你此刻的真实想法或情绪。' : ''}
...
请生成 1 到 3 条回复，每条回复之间用 '|||' 分隔。不要包含其他解释性文字。`;
        ...
        try {
            ...
            for (let i = 0; i < cleanReplies.length; i++) {
                ...
                await typewriterEffect(reply, contact.avatar, thought);
                // 简单的延迟，让多条消息之间有间隔
            ...
            if (imageToSend) {
                const defaultImageUrl = state.defaultVirtualImageUrl || 'https://placehold.co/600x400/png?text=Photo';
                sendMessage(defaultImageUrl, false, 'virtual_image', imageToSend);
            }
    ...
    function typewriterEffect(text, avatarUrl, thought = null) {
        return new Promise(resolve => {
            const container = document.getElementById('chat-messages');
            const msgDiv = document.createElement('div');
            msgDiv.className = 'chat-message other';
            msgDiv.innerHTML = `
                <img src="${avatarUrl}" class="chat-avatar" onclick="window.openAiProfile()" style="cursor: pointer;">
    ...
    async function generateSummary(contact, messages) {
        ...
        const chatText = textMessages.map(m => `${m.role === 'user' ? '我' : contact.name}: ${m.content}`).join('\n');

        const systemPrompt = `你是一个即时通讯软件的聊天记录总结助手。
请阅读以下聊天记录，并提取出其中重要的信息、事实、用户偏好或发生的事件，生成一条简练的“记忆”。
记忆应该是陈述句，包含关键信息。
请务必总结用户和AI聊天的具体内容，不要只总结重要信息。
...
不要包含“聊天记录显示”、“用户说”等前缀，直接陈述事实。`;
    ...
    function handleSaveStickerImport() {
        ...
        if (list.length === 0) {
            alert('未识别到有效的表情包格式，请使用“描述：URL”格式，每行一个');
            return;
        }
        ...
        // 如果是第一个，自动选中
        if (state.stickerCategories.length === 1) {
        ...
        alert(`成功导入 ${list.length} 个表情包`);
    }

    function updateStickerToolbar() {
        const normalToolbar = document.querySelector('.sticker-top-bar');
        const manageToolbar = document.getElementById('sticker-manage-actions');
        const tabBar = document.querySelector('.sticker-tab-bar');
        
        if (state.isStickerManageMode) {
            if (normalToolbar) normalToolbar.classList.add('hidden');
            if (manageToolbar) manageToolbar.classList.remove('hidden');
            if (tabBar) tabBar.classList.add('hidden'); // 管理模式下隐藏底部标签栏
            const countEl = document.getElementById('sticker-select-count');
            if (countEl) countEl.textContent = `已选 ${state.selectedStickers.size}`;
        } else {
            if (normalToolbar) normalToolbar.classList.remove('hidden');
            if (manageToolbar) manageToolbar.classList.add('hidden');
    ...
    function renderStickerPanel() {
        ...
        if (!state.stickerCategories || state.stickerCategories.length === 0) {
            content.innerHTML = '<div style="text-align: center; color: #999; padding: 20px; width: 100%;">暂无表情包，点击左下角 + 号导入</div>';
            return;
        }

        const searchText = document.getElementById('sticker-search-input').value.trim().toLowerCase();
        let stickersToShow = [];

        // 确保有当前分类
        if (!state.currentStickerCategoryId && state.stickerCategories.length > 0) {
        ...
        if (currentCat) {
            currentCat.list.forEach((sticker, index) => {
                // 如果有搜索词，进行过滤；否则全部显示
                if (!searchText || sticker.desc.toLowerCase().includes(searchText)) {
                    stickersToShow.push({
                        ...sticker,
                        catId: currentCat.id,
                        index: index
        ...
        if (stickersToShow.length === 0) {
            content.innerHTML = '<div style="text-align: center; color: #999; padding: 20px; width: 100%;">未找到表情包</div>';
            return;
        }
        ...
        stickersToShow.forEach(sticker => {
            ...
            const uniqueId = `${sticker.catId}-${sticker.index}`;
            
            if (state.isStickerManageMode) {
                const isSelected = state.selectedStickers.has(uniqueId);
                ...
                item.onclick = () => {
                    if (state.selectedStickers.has(uniqueId)) {
                        state.selectedStickers.delete(uniqueId);
                        item.classList.remove('selected');
                    } else {
                        state.selectedStickers.add(uniqueId);
                ...
                item.onclick = () => {
                    // 发送表情包
                    sendMessage(sticker.url, true, 'image');
    ...
    function handleStickerSelectAll() {
        const searchText = document.getElementById('sticker-search-input').value.trim().toLowerCase();
        let allIds = [];
        ...
        if (currentCat) {
            currentCat.list.forEach((sticker, index) => {
                if (!searchText || sticker.desc.toLowerCase().includes(searchText)) {
                    allIds.push(`${currentCat.id}-${index}`);
                }
            });
        }

        // 如果当前已选数量等于当前视图总数，则取消全选，否则全选
        // 这里简化逻辑：只要没全选就全选，全选了就取消
        // 检查当前视图中的ID是否都在 selectedStickers 中
        const allSelected = allIds.every(id => state.selectedStickers.has(id));

        if (allSelected) {
            allIds.forEach(id => state.selectedStickers.delete(id));
        } else {
            allIds.forEach(id => state.selectedStickers.add(id));
        }
    ...
    function handleStickerDelete() {
        if (state.selectedStickers.size === 0) return;

        if (confirm(`确定删除选中的 ${state.selectedStickers.size} 个表情包吗？`)) {
            // 解析选中的ID，按分类分组，并按索引倒序排列（防止删除时索引错位）
            const toDelete = {}; // { catId: [index1, index2] }
            
            state.selectedStickers.forEach(id => {
                const [catId, indexStr] = id.split('-');
            ...
            Object.keys(toDelete).forEach(catId => {
                const cat = state.stickerCategories.find(c => c.id == catId);
                if (cat) {
                    // 倒序删除
                    toDelete[catId].sort((a, b) => b - a).forEach(index => {
                        if (index >= 0 && index < cat.list.length) {
            ...
            state.selectedStickers.clear();
            saveConfig();
            document.getElementById('sticker-select-count').textContent = `已选 0`;
            renderStickerPanel();
        }
    }
    ...
    function renderStickerTabs() {
        ...
        if (!state.stickerCategories) return;

        state.stickerCategories.forEach(cat => {
            const tab = document.createElement('div');
            tab.className = `sticker-tab ${cat.id === state.currentStickerCategoryId ? 'active' : ''}`;
            tab.textContent = cat.name;
            tab.onclick = (e) => {
                e.stopPropagation(); // 阻止冒泡，防止面板关闭
            ...
            tab.addEventListener('contextmenu', (e) => {
                ...
                if (confirm(`确定删除分类 "${cat.name}" 吗？`)) {
                    state.stickerCategories = state.stickerCategories.filter(c => c.id !== cat.id);
                    if (state.currentStickerCategoryId === cat.id) {
