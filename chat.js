...
function renderContactList(filterGroup = 'all') {
    ...
    const renderContent = () => {
        ...
        let filteredContacts = window.iphoneSimState.contacts;
        if (filterGroup !== 'all') {
            filteredContacts = window.iphoneSimState.contacts.filter(c => c.group === filterGroup);
        }
...
**对话节奏**: 你需要模拟真人的聊天习惯，你可以一次性生成多条短消息。每次要回复至少3-8条消息。请用换行符分隔每一条消息。

注意：
- 所有的 ACTION 指令（如发图、转账等）必须单独占一行，放在最后。
- THOUGHT 指令放在最后。
...
- 如果你想给用户拨打语音通话，请在回复最后另起一行输出：ACTION: START_CALL
...
${contact.showThought ? '- **强制执行**：请务必在回复的最后（所有其他内容之后），另起一行输出内心独白。格式：THOUGHT: 内容。' : '- 如果需要输出角色的内心独白（心声），请在回复的最后（所有指令之后），另起一行输出：THOUGHT: 内容。'}
...
7. **THOUGHT: 内容** 是角色的心理活动，用户可见（如果开启了显示）。${contact.showThought ? '当前已开启显示，请务必输出。' : ''}
...
请回复对方的消息。`;
    ...
    try {
        ...
        for (let line of lines) {
            ...
            } else if (trimmedLine.startsWith('THOUGHT:')) {
                thoughtContent = trimmedLine.replace('THOUGHT:', '').trim();
            } else {
        ...
        // 解析 DESC 和 TALK (支持全角冒号，忽略大小写)
        let desc = '';
        let talk = '';
        
        // 预处理：统一冒号
        let processedContent = replyContent.replace(/：/g, ':');
        
        const descMatch = processedContent.match(/DESC:\s*([\s\S]*?)(?=\n?TALK:|$)/i);
        const talkMatch = processedContent.match(/TALK:\s*([\s\S]*?)(?=\n?DESC:|$)/i);
        
        if (descMatch) desc = descMatch[1].trim();
        if (talkMatch) talk = talkMatch[1].trim();
        
        // 回退：如果都没匹配到，且内容不为空，视为 talk
        if (!desc && !talk && replyContent) {
            talk = replyContent;
        }
        ...
        const startCallRegex = /ACTION:\s*START_CALL(?:\s*|$)/;
        ...
        for (let i = 0; i < actions.length; i++) {
            ...
            let startCallMatch;
            while ((startCallMatch = processedSegment.match(startCallRegex)) !== null) {
                setTimeout(() => {
                    startIncomingCall(contact);
                }, 1500);
                processedSegment = processedSegment.replace(startCallMatch[0], '');
            }
        ...
        // 发送 DESC
        if (desc) {
            await typewriterEffect(desc, contact.avatar, null, null, 'description');
        ...
        if (talk) {
            ...
            for (let i = 0; i < lines.length; i++) {
                const line = lines[i];
                const currentThought = (i === lines.length - 1) ? thoughtContent : null;
                const currentReplyTo = (i === 0) ? replyToObj : null;

                // 只有 TALK 部分触发 TTS
                if (contact.ttsEnabled) {
                    // 异步生成语音，不阻塞文字显示
                    generateMinimaxTTS(line, contact.ttsVoiceId).then(audioData => {
                        if (audioData) {
                            // 找到刚才发送的消息并更新音频数据
                            const history = window.iphoneSimState.chatHistory[window.iphoneSimState.currentChatContactId];
                            if (history && history.length > 0) {
                                ...
                                for (let j = history.length - 1; j >= 0; j--) {
                                    const msg = history[j];
                                    if (msg.role === 'assistant' && msg.type === 'text' && msg.content === line) {
                                        // 简单的自动播放逻辑，可能需要优化
                                        const audio = new Audio(audioData);
                                        audio.play().catch(e => console.error('Auto play TTS failed:', e));
                                        break;
                                    }
                                }
                            }
                ...
                await typewriterEffect(line, contact.avatar, currentThought, currentReplyTo, 'text');
                // 模拟打字间隔
                if (i < lines.length - 1) {
...
function typewriterEffect(text, avatarUrl, thought = null, replyTo = null, type = 'text') {
    return new Promise(resolve => {
        ...
        appendMessageToUI(text, false, type, null, replyTo, msgData.id);
...
function setupWhisperListeners() {
    ...
    if (modeSelect && window.iphoneSimState.whisperSettings.url) {
        if (window.iphoneSimState.whisperSettings.url.includes('localhost:3000')) {
    ...
    if (modeSelect) {
        modeSelect.addEventListener('change', (e) => {
            ...
            if (mode === 'local_proxy') {
                ...
                window.iphoneSimState.whisperSettings.url = proxyUrl;
...
window.updateWhisperUi = function() {
    ...
    if (modeSelect && window.iphoneSimState.whisperSettings.url) {
        if (window.iphoneSimState.whisperSettings.url.includes('localhost:3000')) {
            modeSelect.value = 'local_proxy';
        } else if (window.iphoneSimState.whisperSettings.url.includes('corsproxy.io')) {
            modeSelect.value = 'corsproxy';
        } else if (window.iphoneSimState.whisperSettings.url.includes('siliconflow.cn')) {
...
function playVoiceCallAudio(audioData) {
    ...
    globalVoicePlayer.onerror = () => {
        isAiSpeaking = false;
        console.log('AI audio error, VAD resumed');
        if (statusEl) statusEl.textContent = '正在聆听...';
    };

    globalVoicePlayer.play().catch(e => {
...
【重要规则】
1. 你们正在进行视频通话。
2. 请严格遵守以下格式，同时输出一个描述部分和一个对话部分：
{{DESC}}在这里写下你的动作、表情或环境描述。{{/DESC}}
{{DIALOGUE}}在这里写下你以第一人称说的话。{{/DIALOGUE}}
3. 语气要自然、流畅。
4. 如果你想挂断电话，请在回复的最后另起一行输出：ACTION: HANGUP_CALL
...
async function processVoiceCallAudio(audioBlob) {
    ...
    try {
        ...
        if (statusEl && !hasRecognizedText) statusEl.textContent = '正在聆听...'; 
    }
}
